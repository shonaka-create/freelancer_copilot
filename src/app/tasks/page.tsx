...
// Mock PRO analytics (in real app: computed from actual logged time)
const proStats = [
...
export default function TasksPage() {
  ...
  // 1-second tick to force re-render while timers run
  useEffect(() => {
  ...
  // suppress unused warning from tick
  void tick;
  ...
  return (
    <div style={{ maxWidth: '860px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '2rem', padding: '1rem 0' }}>
      ...
      {/* ── PRO: Time Analytics (locked) ── */}
      <div>
        <h2 style={{ fontSize: '1rem', display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '1rem' }}>
          時間単価・工数ウェイト分析
          <span style={{ fontSize: '0.72rem', background: '#DBEAFE', color: 'var(--primary-dark)', padding: '2px 8px', borderRadius: '4px', fontWeight: 700 }}>PRO</span>
        </h2>
                ...
                <tbody>
                  {proStats.map(r => (
                    <tr key={r.label} style={{ borderBottom: '1px solid var(--surface-border)' }}>
                      ...
                      <td style={{ padding: '8px' }}>¥{r.rateYen.toLocaleString()}/h</td>
